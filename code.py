...
def create_rgb_bitmap(pixel_red, pixel_green, pixel_blue, file_name="result.bmp", resolution=[128, 128]):
    ...
    for i in range(resolution[0]):
        for j in range(resolution[1]):
            img_pixels[i, j] = tuple(
                [pixel_red[i + j], pixel_green[i + j], pixel_blue[i + j]])
...
def main():
    ...
    try:
        pixel_red = get_random_integers(
            10000, 0, 255, 1) + get_random_integers(total_pixels - 10000, 0, 255, 1)
        pixel_green = get_random_integers(
            10000, 0, 255, 1) + get_random_integers(total_pixels - 10000, 0, 255, 1)
        pixel_blue = get_random_integers(
            10000, 0, 255, 1) + get_random_integers(total_pixels - 10000, 0, 255, 1)
        create_rgb_bitmap(pixel_red, pixel_green, pixel_blue)
    except:
        return
